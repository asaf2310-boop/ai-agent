...
function matchesKind(job: Job | undefined, kind: MatchPoolFilters["kind"]): boolean {
  ...
  if (kind === "board") {
    return ["alljobs", "drushim", "jobmaster", "jobnet", "gotfriends"].includes(source);
  }
  if (kind === "social") {
    return Boolean(job.is_social) || postKind === "social" || channel.includes("facebook") || channel.includes("group");
  }
  if (kind === "freelance") {
    return postKind === "freelance" || source === "freelance";
  }
  if (kind === "job") {
    return postKind === "job" || (!job.is_social && postKind !== "freelance" && postKind !== "social");
  }
  return true;
}
...
function matchesPostedDate(job: Job | undefined, filters: MatchPoolFilters): boolean {
  const posted = getJobPostedAt(job);
  if (!posted) {
    return filters.postedWithin === "all" && !filters.postedFrom && !filters.postedTo;
  }

  const now = Date.now();

  if (filters.postedWithin === "custom" || filters.postedFrom || filters.postedTo) {
    if (filters.postedFrom && posted < startOfDay(filters.postedFrom)) return false;
    if (filters.postedTo && posted > endOfDay(filters.postedTo)) return false;
    if (filters.postedWithin === "custom" || filters.postedFrom || filters.postedTo) {
      return true;
    }
  }
...
export function filterMatches(matches: JobMatch[], filters: MatchPoolFilters): JobMatch[] {
